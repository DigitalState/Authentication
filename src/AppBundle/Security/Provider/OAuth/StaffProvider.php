 ...
 * Class StaffProvider
 */
class StaffProvider extends FOSUBUserProvider
{
    /**
    ...
    public function loadUserByOAuthUserResponse(UserResponseInterface $response)
    {
        ...
        if ($oAuth) {
            ...
            $registration
                ->setOwner($this->configService->get('app.registration.staff.owner.type'))
                ->setOwnerUuid($this->configService->get('app.registration.staff.owner.uuid'))
                ->setIdentity(Identity::STAFF)
                ->setUsername($email)
                ->setPassword(sha1(uniqid().microtime()))
                ->setData(json_decode(strtr($this->configService->get('app.registration.staff.data.'.$owner), $data), true));
