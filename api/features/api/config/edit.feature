...
Feature: Edit configs
  ...
  Scenario: Edit a config
    ...
    And I send a "PUT" request to "/configs/dc58c698-5af1-4f2c-afe5-43002a15646d" with body:
    """
    {
  ...
  Scenario: Confirm the edited config
    When I add "Accept" header equal to "application/json"
    And I send a "GET" request to "/configs/dc58c698-5af1-4f2c-afe5-43002a15646d"
    Then the response status code should be 200
    And the header "Content-Type" should be equal to "application/json; charset=utf-8"
    And the response should be in JSON
    And the JSON node "value" should be equal to the string "system2@system.ds"
  ...
  Scenario: Edit a config's read-only properties
    ...
    And the JSON node "uuid" should be equal to the string "dc58c698-5af1-4f2c-afe5-43002a15646d"
